...
// Improved gradient parameters
int fadeWidth = 10;  // Width of gradient on each side
float fadeExponent = 1.5;  // Exponent for smoother transition
...
void loadSettings() {
  ...
  // Set reasonable limits for fadeWidth
...
void saveSettings() {
  ...
  // Update fadeWidth after movingLength changes
  fadeWidth = min(fadeWidth, movingLength / 3);
}
...
void handleSetFadeWidth();  // New handler for setting gradient width
...
// New handler for setting gradient width
...
// ------------------------- LED Task with Improved Gradient -------------------------
void ledTask(void * parameter) {
  ...
  for (;;) {
    ...
    // Determine if we should draw the moving light beam
    ...
    if (!lightOn) {
      ...
      // If background mode is active
      if (backgroundModeActive) {
        // Calculate background color once
        CRGB bgColor = CRGB((uint8_t)(baseColor.r * stationaryIntensity),
                            (uint8_t)(baseColor.g * stationaryIntensity),
                            (uint8_t)(baseColor.b * stationaryIntensity));
                            
        // Explicitly fill all LEDs to ensure none are missed
        for (int i = 0; i < NUM_LEDS; i++) {
          leds[i] = bgColor;
        }
      } else {
        // If background mode is not active, start with black
        fill_solid(leds, NUM_LEDS, CRGB::Black);
      }
      ...
      if (drawMovingPart) {
        ...
        // Calculate full brightness color once
        ...
        // Обеспечиваем, что у нас есть направление движения
        int direction = lastMovementDirection;
        if (direction == 0) direction = 1; // По умолчанию направо
        
        // Рассчитываем общую длину световой полосы (основные + дополнительные светодиоды)
        int totalLightLength = movingLength + additionalLEDs;
        
        // Определяем границы всей световой полосы с учетом направления
        ...
        // Ограничиваем края полосы границами ленты
        leftEdge = max(0, leftEdge);
        rightEdge = min(NUM_LEDS - 1, rightEdge);
        
        // Рассчитываем ширину градиента
        int actualFadeWidth = min(fadeWidth, totalLightLength / 3);
        
        // Заполняем центральную часть без градиента
        int nonFadeLeftEdge = leftEdge + actualFadeWidth;
        int nonFadeRightEdge = rightEdge - actualFadeWidth;
        
        // Заполняем центр без градиента
        for (int i = nonFadeLeftEdge; i <= nonFadeRightEdge; i++) {
            if (i >= 0 && i < NUM_LEDS) {
                leds[i] = fullBrightColor;
            }
        }
        
        // Градиент на левом крае (начало света)
        for (int i = 0; i < actualFadeWidth; i++) {
            int idx = leftEdge + i;
            if (idx >= 0 && idx < NUM_LEDS) {
                float normalizedPos = (float)(i + 1) / (actualFadeWidth + 1);
                float factor = 0.1f + 0.9f * pow(normalizedPos, fadeExponent);
        ...
        // Градиент на правом крае (конец света)
        for (int i = 0; i < actualFadeWidth; i++) {
            int idx = rightEdge - i;
            if (idx >= 0 && idx < NUM_LEDS) {
                float normalizedPos = (float)(i + 1) / (actualFadeWidth + 1);
    ...
    // This line should always be after processing all LEDs
    FastLED.show();
...
void setupWiFi() {
  ...
  // Generate unique access point name with 3 random characters
  randomSeed(ESP.getEfuseMac());
  char suffix[4];
  for (int i = 0; i < 3; i++) {
    int r = random(0, 36); // 0-35: digits and uppercase letters
    if (r < 10)
      suffix[i] = '0' + r;
    else
      suffix[i] = 'A' + (r - 10);
  ...
  suffix[3] = '\0';
  String deviceName = "LightTrack " + String(suffix);
  ...
  server.on("/setFadeWidth", handleSetFadeWidth); // Add new handler
...
void handleRoot() {
  ...
  String html = "<html><head><title>LED Control</title>"
    ...
    "<style>"
      ...
      ".section-header { margin-top: 20px; background-color: #444; padding: 5px; border-radius: 4px; }"
    "</style>"
    "<script>"
      ...
      "function setFadeWidth(val) { fetch('/setFadeWidth?value=' + val); }"
    ...
    "<div class='container'>"
      ...
      "<div class='section-header'>Main Parameters</div>"
      ...
      "<div class='section-header'>Gradient Parameters</div>"
      "<p>Fade Width (gradient): <span id='fadeWidthValue'>" + String(fadeWidth) + "</span></p>"
      "<input type='range' min='1' max='" + String(max(1, movingLength/3)) + "' step='1' value='" + String(fadeWidth) + "' oninput='document.getElementById(\"fadeWidthValue\").innerText = this.value' onchange='setFadeWidth(this.value)'>"
      
      "<div class='section-header'>Additional Parameters</div>"
      ...
      "<div class='section-header'>Background Mode</div>"
      "<p>Background Light Mode:</p>"
      "<button onclick='toggleBackgroundMode()'>" + String(backgroundModeActive ? "Turn Off Background" : "Turn On Background") + "</button>"
...
void handleSetMovingLength() {
  if (server.hasArg("value")) { 
    ...
    // Update fadeWidth to ensure it's compatible with the new moving length
    fadeWidth = min(fadeWidth, movingLength / 3);
  }
  saveSettings();
  server.sendHeader("Location", "/");
